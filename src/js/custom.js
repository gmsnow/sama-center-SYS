...
async function init_sparklines() {
    ...
    // Fetch daily income from backend
    let weeklyData = Array(7).fill(0); // default 0
    try {
        const response = await fetch('/api/weekly-Summary');
        if (response.ok) {
            const data = await response.json();
            weeklyData = data.dailyTotals || weeklyData;
        }
    } catch (err) {
        console.error('Error fetching weekly daily income:', err);
    }

    // Render the weekly income sparkline (replace the static array)
    $(".sparkline_one").sparkline(weeklyData, {
        type: 'bar',          // or 'line' if you prefer line chart
        height: '125',
        barWidth: 13,
        colorMap: {
            '7': '#a1a1a1'
    ...
    // Other sparklines remain as before
    ...
    // ... keep other sparkline initializations as they are
